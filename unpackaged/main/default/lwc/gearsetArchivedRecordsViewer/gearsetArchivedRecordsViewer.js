...
import { API_VERSION, isApiMessageEvent } from "./message-api.js";
import { CurrentPageReference as CurrentPageReferenceWireAdapter, NavigationMixin } from "lightning/navigation";
import { navigate } from "./gearsetArchivedRecordsViewerHelper.js";
const defaultPage = {
    standard__recordPage: "archive-viewer",
    standard__navItemPage: "default-tab"
};
export default class GearsetArchivedRecordsViewer extends NavigationMixin(LightningElement) {
    @api
    recordId;
    @api
    objectApiName;
    ...
    connectedCallback() {
        const route = this.route ??
            ...
            "archive-viewer";
        this.iframeUrl = `https://embed.gearset.com/${route}?${new URLSearchParams({
            v: API_VERSION,
    ...
    handleMessageEvent = (event) => {
        if (event.origin !== "https://embed.gearset.com" || !isApiMessageEvent(event)) {
            return;
        }
    ...
    async handleApiMessageEvent(event) {
